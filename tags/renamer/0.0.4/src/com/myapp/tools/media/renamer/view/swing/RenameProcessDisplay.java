...
class RenameProcessDisplay implements IRenameProcessListener {
    ...
    public void processFileStart(IRenamable file) {
        ...
        String statusLabelText = Msg.msg("RenameProcessReport.label.status")
                    .replace("#oldName#", renamedAtTheMoment.getOldName())
                    .replace("#newName#", renamedAtTheMoment.getNewName())
                    .replace("#statusStr#", processFigure);

        statusLabel.setText(statusLabelText);
        frame.setTitle(Msg.msg("RenameProcessReport.title.status")
                        .replace("#statusStr#", processFigure));
    ...
    private String processFigure() {
        return Msg.msg("RenameProcessReport.statusString")
                    .replace("#renamed#", Integer.toString(totalRenamed + 1))
                    .replace("#total#", Integer.toString(totalCount));
    }
}