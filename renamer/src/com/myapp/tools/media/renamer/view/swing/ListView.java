...
final class ListView extends JPanel implements IListView, ISysConstants {
    ...
    private void createTable() {
        jtable = new JTable(app.getTableModel()) {{
            ...
            addMouseListener(new MouseAdapter() {
                public void mouseClicked(MouseEvent e) {
                    ...
                    // make sure that click was on a selected row:
        ...
        public Component prepareRenderer(TableCellRenderer renderer,
            ...
            jc.setToolTipText(msg("Table.tableContextMenu.text")
                    .replace("#oldname#", rn.getOldName())
                    .replace("#newname#", rn.getNewName())
                    .replace("#oldparent#", rn.getOldParentAbsolutePath())
                    .replace("#newparent#", rn.getNewParentAbsolutePath())
                    .replace("#size#", Util.getHumanReadableFileSize(
                                               rn.getSourceObject().length())));
            return c;
        }
...
//        reset.setActionCommand(DISCARD_SELECTIONS_CHANGES);
        ...
        popupMenu.show((Component) app.getUIComponent(),
                       pointInJTable.x,
                       pointInJTable.y + 50);
    }
    ...
    public void persistSettings(IRenamerConfiguration cfg) {
        ...
        for (int i = 0, count = m.getColumnCount(); i < count; i++) {
            col = m.getColumn(i);
            name = col.getHeaderValue().toString();    // FIXME: pretty ugly !
