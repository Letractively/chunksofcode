...
private def int generateTestData(String databaseTable) {
    ...
    final int createLines = 6 * 365 * 24 * 4
...
private def void createTable(String tablename) {
    ...
    create index fmcontrol_kw_leistung_idx_datum on `$tablename` (datum);
    create index fmcontrol_kw_leistung_idx_zeit on `$tablename` (zeit);
    """
    println()
}
...
System.err.println "starting with script!"
println "-- init script for table fmcontrol_kw_leistung start --"
println "-- author: andre ragg ${new java.text.SimpleDateFormat('yyyy').format(new Date())} --"
println()
System.err.println "-- creating tables --"; println "-- creating tables --"

createTable("fmcontrol_kw_leistung")
createTable("fmcontrol_kw_leistung2")

println """
create view nasenview 
as select * from fmcontrol_kw_leistung 
union all 
select * from fmcontrol_kw_leistung2;
"""
...
// println """
// alter table fmcontrol_kw_leistung add monat int not null default year(datum);
// update fmcontrol_kw_leistung set monat = month(datum);
// create index fmcontrol_kw_leistung_idx_monat on fmcontrol_kw_leistung(monat);
//
// alter table fmcontrol_kw_leistung add jahr int;
// update fmcontrol set jahr = year(datum);
// create index fmcontrol_kw_leistung_idx_jahr on fmcontrol_kw_leistung(jahr);
// """
...
int globalCount = 0
globalCount += generateTestData("fmcontrol_kw_leistung")
globalCount += generateTestData("fmcontrol_kw_leistung2")
