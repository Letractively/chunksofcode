...
public class LinuxProcStateMgr implements IProcessManager {
    ...
    @Override
    public Map<Integer, Proc> determineProcessStates() {
        Map<Integer, Proc> map = new TreeMap<Integer, Proc>();
        String state = IO.readProcOutput("bash", "-c", "ps auxww | grep -v grep | grep -E \"^\\s*$(whoami)\"");
    ...
    private static final Pattern PS_AUXWWW_OUTPUT_PATTERN = Pattern.compile( 
        "(?ix)  " +
        "^ \\s* "+
        Pattern.quote(Config.getInstance().getUsername())+" \\s+ "+
