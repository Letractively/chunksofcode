...
public class BlockIterator {
    ...
    void forEachTile(Block b, BlockTileVisitor v) {
        for (int relRow = 0; relRow < b.flags.length; relRow++) {
            ...
            for (int relCol = 0; relCol < b.flags[0].length; relCol++) {
                ...
                if (absCol < 0 
                        || absCol >= b.game.columns 
                        || ! b.flags[relRow][relCol]) {
                    continue;
                } 
    ...
    static class VisibleBlockTileIterator implements Iterator<Tile> { // TODO
        ...
        private Tile searchNext() {
            final int flagsLen = b.flags.length; // note: b.flags is a square!
            
            for (; relativeRow < flagsLen; relativeRow++) {
                int absRow = calcAbsRow();

                if (absRow < 0 || absRow >= b.game.rows) {
                    // this row is outside of the game bounds
                    continue; 
                }
                ...
                for (; relativeCol < flagsLen; relativeCol++) {
                    ...
                    if (absCol < 0 
                            || absCol >= b.game.columns 
                            || ! b.flags[relativeRow][relativeCol]) {
                        // this col is outside of the game bounds
                        continue;
                    }

                    // set cursors to next position 
                    // and return corresponding tile:
    ...
    void forEachBottomTile(final Block b, final BlockTileVisitor v) {
        forEachTile(b, new BlockTileVisitor() {
            public boolean visit(Tile tile, int relR, int relC) {
                if (relR == b.flags.length - 1 ) { // at bottom row
                    return v.visit(tile, relR, relC);
                }
                if (! b.flags[relR + 1][relC]) { // no item below
                    return v.visit(tile, relR, relC);
                }
                return true;
            }
