...
//        args = testArgs();
        ...
        try {
            ...
            PrintStream logOut = (logFile == null)
                            ? System.out
    ...
    private static String[] testArgs()  {
        try {
            Runtime.getRuntime().exec(new String[] {
               ...
               "rm -r /tmp/nasen;        " +
               "mkdir /tmp/nasen;        " +
               "echo a > /tmp/nasen/a;   " +    /*duplicate*/
               "echo b > /tmp/nasen/b;   " +
               "echo c > /tmp/nasen/c;   " +
               "mkdir /tmp/nasen/foo;    " +
               "echo a > /tmp/nasen/foo/d"      /*duplicate*/
            }).waitFor();     
        ...
        String[] testArgs = {
            ...
            CMD_GENERATE_SCRIPT,
//            CMD_EXECUTE_SCRIPT,
            PARAM_PREFIX_ROOT_DIR +    "/tmp/nasen",
//            PARAM_PREFIX_HASH_FILE +   "/home/andre/Desktop/hashFile.sha1",
            PARAM_PREFIX_CRUNCH_SCRIPT+"/home/andre/Desktop/crunch.sh",
//            PARAM_PREFIX_LOG_FILE +    "/home/andre/Desktop/crunch.log",
            "--verbose"
        };
        return testArgs;
    }
    ...
    private void hash() {
        if (rootDir == null || ! rootDir.isDirectory() || ! rootDir.canRead())
            throw new RuntimeException("not a readable directory: " + rootDir);
    ...
    private void generate() {
        if ( ! hashFile.isFile() || ! hashFile.canRead())
            throw new RuntimeException("not a readable file: " + hashFile);
        if (crunchScript == null)
            throw new RuntimeException("not a writeable file: " + crunchScript);
    ...
    private static File parseFile(String prefix, String[] args) {
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            if (param.startsWith(prefix)) {
                if (path != null)
                    throw new RuntimeException("parameter "+prefix+"specified more than once!");
    ...
    p("");
    p("EXAMPLES:");
    p(" "+CMD_CREATE_HASHFILE+" "+PARAM_PREFIX_ROOT_DIR+"/media/sound "+
                                   PARAM_PREFIX_HASH_FILE+"/tmp/hashfile.txt");
    p("      creates a hashfile for the soundcollection.");
    p("");
    p(" "+CMD_GENERATE_SCRIPT+" "+PARAM_PREFIX_HASH_FILE+"/tmp/hashfile.txt "+
                                   PARAM_PREFIX_CRUNCH_SCRIPT+"/tmp/crunch.sh");
    p("      generates a crunch-script based on the given hashfile.");
    p("");
    p(" "+CMD_CREATE_HASHFILE+"+"+CMD_GENERATE_SCRIPT+" "+
                                   PARAM_PREFIX_ROOT_DIR+"/media/sound "+
                                   PARAM_PREFIX_CRUNCH_SCRIPT+"/tmp/crunch.sh");
