        ...
        try {
            ...
            if (exitstatus != 0) {
                throw new RuntimeException(
                    "process "+pb.command()+" finished with status: "+exitstatus+"."+NL+
                    "---output start---"+NL+stdOut+NL+"---output end---"+NL+
                    "---error start---"+NL+errOut+NL+"---error end---"
                );
            }
            ...
            throw new RuntimeException(
                "could not execute: " + pb.command() + NL +
                "--- ERROR OUTPUT START ---" + NL +
                errOut.toString() + NL + 
                "--- ERROR OUTPUT END ---"
                , e
            );
